...
void rlScalef(float x, float y, float z)
{
    ...
    // NOTE: Transposing matrix by multiplication order
    *RLVK.State.currentMatrix = rlMatrixMultiply(matScale, *RLVK.State.currentMatrix);
}
...
void rlMultMatrixf(const float *matf)
{
    ...
    *RLVK.State.currentMatrix = rlMatrixMultiply(mat, *RLVK.State.currentMatrix);
}
...
void rlSetTexture(unsigned int id)
{
    ...
    else
    {
        ...
        rlEnableTexture(id);
#else
        RLVK.State.currentTextureId = id;
        if (RLVK.currentBatch->draws[RLVK.currentBatch->drawCounter - 1].textureId != id)
        {
            if (RLVK.currentBatch->draws[RLVK.currentBatch->drawCounter - 1].vertexCount > 0)
            {
                // Make sure current RLVK.currentBatch->draws[i].vertexCount is aligned a multiple of 4,
            ...
            RLVK.currentBatch->draws[RLVK.currentBatch->drawCounter - 1].textureId = id;
...
void rlTextureParameters(unsigned int id, int param, int value)
{
    ...
    switch (param)
    {
        ...
        case RL_TEXTURE_WRAP_T:
        {
            ...
            {
#if !defined(GRAPHICS_API_OPENGL_11)
                if (RLVK.ExtSupported.texMirrorClamp) glTexParameteri(GL_TEXTURE_2D, param, value);
        ...
        case RL_TEXTURE_FILTER_ANISOTROPIC:
        {
            ...
            if (value <= RLVK.ExtSupported.maxAnisotropyLevel) glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, (float)value);
            else if (RLVK.ExtSupported.maxAnisotropyLevel > 0.0f)
            {
                TRACELOG(RL_LOG_WARNING, "GL: Maximum anisotropic filter level supported is %iX", id, (int)RLVK.ExtSupported.maxAnisotropyLevel);
...
void rlCubemapParameters(unsigned int id, int param, int value)
{
    ...
    switch (param)
    {
        ...
        case RL_TEXTURE_WRAP_T:
        {
            if (value == RL_TEXTURE_WRAP_MIRROR_CLAMP)
            {
                if (RLVK.ExtSupported.texMirrorClamp) glTexParameteri(GL_TEXTURE_CUBE_MAP, param, value);
                else TRACELOG(RL_LOG_WARNING, "GL: Clamp mirror wrap mode not supported (GL_MIRROR_CLAMP_EXT)");
            }
        ...
        case RL_TEXTURE_FILTER_ANISOTROPIC:
        {
            if (value <= RLVK.ExtSupported.maxAnisotropyLevel) glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAX_ANISOTROPY_EXT, (float)value);
            else if (RLVK.ExtSupported.maxAnisotropyLevel > 0.0f)
            {
                TRACELOG(RL_LOG_WARNING, "GL: Maximum anisotropic filter level supported is %iX", id, (int)RLVK.ExtSupported.maxAnisotropyLevel);
...
{
#if (defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2))
    return RLVK.State.stereoRender;
#else
    return false;
#endif
}
...
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if ((RLVK.State.currentBlendMode != mode) || ((mode == RL_BLEND_CUSTOM || mode == RL_BLEND_CUSTOM_SEPARATE) && RLVK.State.glCustomBlendModeModified))
    {
        rlDrawRenderBatch(RLVK.currentBatch);
        ...
        switch (mode)
        {
            ...
            case RL_BLEND_CUSTOM:
            {
                ...
                glBlendFunc(RLVK.State.glBlendSrcFactor, RLVK.State.glBlendDstFactor); glBlendEquation(RLVK.State.glBlendEquation);
            } break;
            case RL_BLEND_CUSTOM_SEPARATE:
            {
                // NOTE: Using GL blend src/dst factors and GL equation configured with rlSetBlendFactorsSeparate()
                glBlendFuncSeparate(RLVK.State.glBlendSrcFactorRGB, RLVK.State.glBlendDestFactorRGB, RLVK.State.glBlendSrcFactorAlpha, RLVK.State.glBlendDestFactorAlpha);
                glBlendEquationSeparate(RLVK.State.glBlendEquationRGB, RLVK.State.glBlendEquationAlpha);
            } break;
            default: break;
        }

        RLVK.State.currentBlendMode = mode;
        RLVK.State.glCustomBlendModeModified = false;
    }
#endif
}
...
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if ((RLVK.State.glBlendSrcFactor != glSrcFactor) ||
        (RLVK.State.glBlendDstFactor != glDstFactor) ||
        (RLVK.State.glBlendEquation != glEquation))
    {
        RLVK.State.glBlendSrcFactor = glSrcFactor;
        RLVK.State.glBlendDstFactor = glDstFactor;
        RLVK.State.glBlendEquation = glEquation;

        RLVK.State.glCustomBlendModeModified = true;
    }
#endif
}
...
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if ((RLVK.State.glBlendSrcFactorRGB != glSrcRGB) ||
        (RLVK.State.glBlendDestFactorRGB != glDstRGB) ||
        (RLVK.State.glBlendSrcFactorAlpha != glSrcAlpha) ||
        (RLVK.State.glBlendDestFactorAlpha != glDstAlpha) ||
        (RLVK.State.glBlendEquationRGB != glEqRGB) ||
        (RLVK.State.glBlendEquationAlpha != glEqAlpha))
    {
        RLVK.State.glBlendSrcFactorRGB = glSrcRGB;
        RLVK.State.glBlendDestFactorRGB = glDstRGB;
        RLVK.State.glBlendSrcFactorAlpha = glSrcAlpha;
        RLVK.State.glBlendDestFactorAlpha = glDstAlpha;
        RLVK.State.glBlendEquationRGB = glEqRGB;
...
//----------------------------------------------------------------------------------
#if defined(GRAPHICS_API_OPENGL_43) && RLVK_ENABLE_VULKAN_DEBUG_CONTEXT
...
//----------------------------------------------------------------------------------
// Module Functions Definition - rlvk functionality
//----------------------------------------------------------------------------------

// Initialize rlvk: OpenGL extensions, default buffers/shaders/textures, OpenGL states
void rlvkInit(int width, int height)
{
    isGpuReady = true;
    ...
    RLVK.State.defaultTextureId = rlLoadTexture(pixels, 1, 1, RL_PIXELFORMAT_UNCOMPRESSED_R8G8B8A8, 1);
    RLVK.State.currentTextureId = RLVK.State.defaultTextureId;

    if (RLVK.State.defaultTextureId != 0) TRACELOG(RL_LOG_INFO, "TEXTURE: [ID %i] Default texture loaded successfully", RLVK.State.defaultTextureId);
    else TRACELOG(RL_LOG_WARNING, "TEXTURE: Failed to load default texture");

    // Init default Shader (customized for GL 3.3 and ES2)
    // Loaded: RLVK.State.defaultShaderId + RLVK.State.defaultShaderLocs
    rlLoadShaderDefault();
    RLVK.State.currentShaderId = RLVK.State.defaultShaderId;
    RLVK.State.currentShaderLocs = RLVK.State.defaultShaderLocs;
    ...
    RLVK.State.currentShaderLocs[RL_SHADER_LOC_VERTEX_NORMAL] = RL_DEFAULT_SHADER_ATTRIB_LOCATION_NORMAL;
    RLVK.defaultBatch = rlLoadRenderBatch(RL_DEFAULT_BATCH_BUFFERS, RL_DEFAULT_BATCH_BUFFER_ELEMENTS);
    RLVK.State.currentShaderLocs[RL_SHADER_LOC_VERTEX_NORMAL] = -1;
    ...
    TRACELOG(RL_LOG_INFO, "RLVK: Default Vulkan state initialized successfully");
    //----------------------------------------------------------
}
...
void rlvkClose(void)
{
    ...
    glDeleteTextures(1, &RLVK.State.defaultTextureId); // Unload default texture
    TRACELOG(RL_LOG_INFO, "TEXTURE: [ID %i] Default texture unloaded successfully", RLVK.State.defaultTextureId);
#endif
...
void rlLoadExtensions(void *loader)
{
    ...
    RLVK.ExtSupported.vao = GLAD_GL_ARB_vertex_array_object;
    RLVK.ExtSupported.instancing = (GLAD_GL_EXT_draw_instanced && GLAD_GL_ARB_instanced_arrays);
    RLVK.ExtSupported.texNPOT = GLAD_GL_ARB_texture_non_power_of_two;
    RLVK.ExtSupported.texFloat32 = GLAD_GL_ARB_texture_float;
    RLVK.ExtSupported.texFloat16 = GLAD_GL_ARB_texture_float;
    RLVK.ExtSupported.texDepth = GLAD_GL_ARB_depth_texture;
    RLVK.ExtSupported.maxDepthBits = 32;
    RLVK.ExtSupported.texAnisoFilter = GLAD_GL_EXT_texture_filter_anisotropic;
    RLVK.ExtSupported.texMirrorClamp = GLAD_GL_EXT_texture_mirror_clamp;
#else
    // Register supported extensions flags
    // OpenGL 3.3 extensions supported by default (core)
    RLVK.ExtSupported.vao = true;
    RLVK.ExtSupported.instancing = true;
    RLVK.ExtSupported.texNPOT = true;
    RLVK.ExtSupported.texFloat32 = true;
    RLVK.ExtSupported.texFloat16 = true;
    RLVK.ExtSupported.texDepth = true;
    RLVK.ExtSupported.maxDepthBits = 32;
    RLVK.ExtSupported.texAnisoFilter = true;
    RLVK.ExtSupported.texMirrorClamp = true;
#endif

    // Optional OpenGL 3.3 extensions
    RLVK.ExtSupported.texCompASTC = GLAD_GL_KHR_texture_compression_astc_hdr && GLAD_GL_KHR_texture_compression_astc_ldr;
    RLVK.ExtSupported.texCompDXT = GLAD_GL_EXT_texture_compression_s3tc;  // Texture compression: DXT
    RLVK.ExtSupported.texCompETC2 = GLAD_GL_ARB_ES3_compatibility;        // Texture compression: ETC2/EAC
    #if defined(GRAPHICS_API_OPENGL_43)
    RLVK.ExtSupported.computeShader = GLAD_GL_ARB_compute_shader;
    RLVK.ExtSupported.ssbo = GLAD_GL_ARB_shader_storage_buffer_object;
    #endif
    ...
    RLVK.ExtSupported.texFloat16 = true;
    RLVK.ExtSupported.texDepth = true;
    RLVK.ExtSupported.texDepthWebGL = true;
    RLVK.ExtSupported.maxDepthBits = 24;
    RLVK.ExtSupported.texAnisoFilter = true;
    RLVK.ExtSupported.texMirrorClamp = true;
    // TODO: Check for additional OpenGL ES 3.0 supported extensions:
    //RLVK.ExtSupported.texCompDXT = true;
    //RLVK.ExtSupported.texCompETC1 = true;
    //RLVK.ExtSupported.texCompETC2 = true;
    //RLVK.ExtSupported.texCompPVRT = true;
    //RLVK.ExtSupported.texCompASTC = true;
    //RLVK.ExtSupported.maxAnisotropyLevel = true;
    //RLVK.ExtSupported.computeShader = true;
    //RLVK.ExtSupported.ssbo = true;
    ...
    for (int i = 0; i < numExt; i++)
    {
        ...
        if (strcmp(extList[i], (const char *)"GL_OES_vertex_array_object") == 0)
        {
            ...
            if ((glGenVertexArrays != NULL) && (glBindVertexArray != NULL) && (glDeleteVertexArrays != NULL)) RLVK.ExtSupported.vao = true;
        }
        ...
        if (strstr(extList[i], (const char *)"instanced_arrays") != NULL)   // Broad check for instanced_arrays
        {
            ...
            if ((glDrawArraysInstanced != NULL) && (glDrawElementsInstanced != NULL) && (glVertexAttribDivisor != NULL)) RLVK.ExtSupported.instancing = true;
        }
        ...
        if (strcmp(extList[i], (const char *)"GL_OES_texture_npot") == 0) RLVK.ExtSupported.texNPOT = true;

        // Check texture float support
        if (strcmp(extList[i], (const char *)"GL_OES_texture_float") == 0) RLVK.ExtSupported.texFloat32 = true;
        if (strcmp(extList[i], (const char *)"GL_OES_texture_half_float") == 0) RLVK.ExtSupported.texFloat16 = true;

        // Check depth texture support
        if (strcmp(extList[i], (const char *)"GL_OES_depth_texture") == 0) RLVK.ExtSupported.texDepth = true;
        if (strcmp(extList[i], (const char *)"GL_WEBGL_depth_texture") == 0) RLVK.ExtSupported.texDepthWebGL = true; // WebGL requires unsized internal format
        if (RLVK.ExtSupported.texDepthWebGL) RLVK.ExtSupported.texDepth = true;

        if (strcmp(extList[i], (const char *)"GL_OES_depth24") == 0) RLVK.ExtSupported.maxDepthBits = 24; // Not available on WebGL
        if (strcmp(extList[i], (const char *)"GL_OES_depth32") == 0) RLVK.ExtSupported.maxDepthBits = 32; // Not available on WebGL

        // Check texture compression support: DXT
        if ((strcmp(extList[i], (const char *)"GL_EXT_texture_compression_s3tc") == 0) ||
            (strcmp(extList[i], (const char *)"GL_WEBGL_compressed_texture_s3tc") == 0) ||
            (strcmp(extList[i], (const char *)"GL_WEBKIT_WEBGL_compressed_texture_s3tc") == 0)) RLVK.ExtSupported.texCompDXT = true;

        // Check texture compression support: ETC1
        if ((strcmp(extList[i], (const char *)"GL_OES_compressed_ETC1_RGB8_texture") == 0) ||
            (strcmp(extList[i], (const char *)"GL_WEBGL_compressed_texture_etc1") == 0)) RLVK.ExtSupported.texCompETC1 = true;

        // Check texture compression support: ETC2/EAC
        if (strcmp(extList[i], (const char *)"GL_ARB_ES3_compatibility") == 0) RLVK.ExtSupported.texCompETC2 = true;

        // Check texture compression support: PVR
        if (strcmp(extList[i], (const char *)"GL_IMG_texture_compression_pvrtc") == 0) RLVK.ExtSupported.texCompPVRT = true;

        // Check texture compression support: ASTC
        if (strcmp(extList[i], (const char *)"GL_KHR_texture_compression_astc_hdr") == 0) RLVK.ExtSupported.texCompASTC = true;

        // Check anisotropic texture filter support
        if (strcmp(extList[i], (const char *)"GL_EXT_texture_filter_anisotropic") == 0) RLVK.ExtSupported.texAnisoFilter = true;

        // Check clamp mirror wrap mode support
        if (strcmp(extList[i], (const char *)"GL_EXT_texture_mirror_clamp") == 0) RLVK.ExtSupported.texMirrorClamp = true;
    }
...
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    RLVK.loader = (rlglLoadProc)loader;

    // NOTE: Anisotropy levels capability is an extension
    #ifndef GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT
        #define GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT 0x84FF
    #endif
    glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &RLVK.ExtSupported.maxAnisotropyLevel);
    ...
    if (RLVK.ExtSupported.texAnisoFilter) TRACELOG(RL_LOG_INFO, "    GL_MAX_TEXTURE_MAX_ANISOTROPY: %.0f", RLVK.ExtSupported.maxAnisotropyLevel);
    #endif
...
#else   // !RLVK_SHOW_VK_DETAILS_INFO

    // Show some basic info about GL supported features
    if (RLVK.ExtSupported.vao) TRACELOG(RL_LOG_INFO, "GL: VAO extension detected, VAO functions loaded successfully");
    else TRACELOG(RL_LOG_WARNING, "GL: VAO extension not found, VAO not supported");
    if (RLVK.ExtSupported.texNPOT) TRACELOG(RL_LOG_INFO, "GL: NPOT textures extension detected, full NPOT textures supported");
    else TRACELOG(RL_LOG_WARNING, "GL: NPOT textures extension not found, limited NPOT support (no-mipmaps, no-repeat)");
    if (RLVK.ExtSupported.texCompDXT) TRACELOG(RL_LOG_INFO, "GL: DXT compressed textures supported");
    if (RLVK.ExtSupported.texCompETC1) TRACELOG(RL_LOG_INFO, "GL: ETC1 compressed textures supported");
    if (RLVK.ExtSupported.texCompETC2) TRACELOG(RL_LOG_INFO, "GL: ETC2/EAC compressed textures supported");
    if (RLVK.ExtSupported.texCompPVRT) TRACELOG(RL_LOG_INFO, "GL: PVRT compressed textures supported");
    if (RLVK.ExtSupported.texCompASTC) TRACELOG(RL_LOG_INFO, "GL: ASTC compressed textures supported");
    if (RLVK.ExtSupported.computeShader) TRACELOG(RL_LOG_INFO, "GL: Compute shaders supported");
    if (RLVK.ExtSupported.ssbo) TRACELOG(RL_LOG_INFO, "GL: Shader storage buffer objects supported");
#endif
...
int rlGetFramebufferWidth(void)
{
    int width = 0;
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    width = RLVK.State.framebufferWidth;
#endif
    return width;
}
...
int rlGetFramebufferHeight(void)
{
    int height = 0;
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    height = RLVK.State.framebufferHeight;
#endif
    return height;
}
...
unsigned int rlGetTextureIdDefault(void)
{
    unsigned int id = 0;
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    id = RLVK.State.defaultTextureId;
#endif
    return id;
}
...
rlRenderBatch rlLoadRenderBatch(int numBuffers, int bufferElements)
{
    ...
    if (RLVK.ExtSupported.vao) glBindVertexArray(0);
    //--------------------------------------------------------------------------------------------
...
void rlUnloadRenderBatch(rlRenderBatch batch)
{
    ...
    for (int i = 0; i < batch.bufferCount; i++)
    {
        ...
        if (RLVK.ExtSupported.vao) glDeleteVertexArrays(1, &batch.vertexBuffer[i].vaoId);
...
void rlDrawRenderBatch(rlRenderBatch *batch)
{
    ...
    if (RLVK.State.vertexCounter > 0)
    {
        ...
        if (RLVK.ExtSupported.vao) glBindVertexArray(batch->vertexBuffer[batch->currentBuffer].vaoId);
        ...
        glBufferSubData(GL_ARRAY_BUFFER, 0, RLVK.State.vertexCounter*3*sizeof(float), batch->vertexBuffer[batch->currentBuffer].vertices);
        //glBufferData(GL_ARRAY_BUFFER, sizeof(float)*3*4*batch->vertexBuffer[batch->currentBuffer].elementCount, batch->vertexBuffer[batch->currentBuffer].vertices, GL_DYNAMIC_DRAW);  // Update all buffer

        // Texture coordinates buffer
        glBindBuffer(GL_ARRAY_BUFFER, batch->vertexBuffer[batch->currentBuffer].vboId[1]);
        glBufferSubData(GL_ARRAY_BUFFER, 0, RLVK.State.vertexCounter*2*sizeof(float), batch->vertexBuffer[batch->currentBuffer].texcoords);
        //glBufferData(GL_ARRAY_BUFFER, sizeof(float)*2*4*batch->vertexBuffer[batch->currentBuffer].elementCount, batch->vertexBuffer[batch->currentBuffer].texcoords, GL_DYNAMIC_DRAW); // Update all buffer

        // Normals buffer
        glBindBuffer(GL_ARRAY_BUFFER, batch->vertexBuffer[batch->currentBuffer].vboId[2]);
        glBufferSubData(GL_ARRAY_BUFFER, 0, RLVK.State.vertexCounter*3*sizeof(float), batch->vertexBuffer[batch->currentBuffer].normals);
        ...
        glBufferSubData(GL_ARRAY_BUFFER, 0, RLVK.State.vertexCounter*4*sizeof(unsigned char), batch->vertexBuffer[batch->currentBuffer].colors);
    ...
    for (int eye = 0; eye < eyeCount; eye++)
    {
        ...
        if (RLVK.ExtSupported.vao) glBindVertexArray(0); // Unbind VAO
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format, int mipmapCount)
{
...
#else
    if ((!RLVK.ExtSupported.texCompDXT) && ((format == RL_PIXELFORMAT_COMPRESSED_DXT1_RGB) || (format == RL_PIXELFORMAT_COMPRESSED_DXT1_RGBA) ||
...
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if ((!RLVK.ExtSupported.texCompETC1) && (format == RL_PIXELFORMAT_COMPRESSED_ETC1_RGB))
    {
        TRACELOG(RL_LOG_WARNING, "GL: ETC1 compressed texture format not supported");
        return id;
    }

    if ((!RLVK.ExtSupported.texCompETC2) && ((format == RL_PIXELFORMAT_COMPRESSED_ETC2_RGB) || (format == RL_PIXELFORMAT_COMPRESSED_ETC2_EAC_RGBA)))
    {
        TRACELOG(RL_LOG_WARNING, "GL: ETC2 compressed texture format not supported");
        return id;
    }

    if ((!RLVK.ExtSupported.texCompPVRT) && ((format == RL_PIXELFORMAT_COMPRESSED_PVRT_RGB) || (format == RL_PIXELFORMAT_COMPRESSED_PVRT_RGBA)))
    {
        TRACELOG(RL_LOG_WARNING, "GL: PVRT compressed texture format not supported");
        return id;
    }

    if ((!RLVK.ExtSupported.texCompASTC) && ((format == RL_PIXELFORMAT_COMPRESSED_ASTC_4x4_RGBA) || (format == RL_PIXELFORMAT_COMPRESSED_ASTC_8x8_RGBA)))
    {
    ...
    if ((texIsPOT) || (RLVK.ExtSupported.texNPOT))
    {
...
unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer)
{
    ...
    if (!RLVK.ExtSupported.texDepth) useRenderBuffer = true;
    ...
    if (!RLVK.ExtSupported.texDepthWebGL || useRenderBuffer)
    {
        if (RLVK.ExtSupported.maxDepthBits == 32) glInternalFormat = GL_DEPTH_COMPONENT32_OES;
        else if (RLVK.ExtSupported.maxDepthBits == 24) glInternalFormat = GL_DEPTH_COMPONENT24_OES;
    ...
    if (RLVK.ExtSupported.maxDepthBits == 24) glInternalFormat = GL_DEPTH_COMPONENT24;
    else glInternalFormat = GL_DEPTH_COMPONENT16;
#endif

    if (!useRenderBuffer && RLVK.ExtSupported.texDepth)
    {
        glGenTextures(1, &id);
        glBindTexture(GL_TEXTURE_2D, id);
    ...
    else
    {
        ...
        TRACELOG(RL_LOG_INFO, "TEXTURE: [ID %i] Depth renderbuffer loaded successfully (%i bits)", id, (RLVK.ExtSupported.maxDepthBits >= 24)? RLVK.ExtSupported.maxDepthBits : 16);
    }
...
void rlGetGlTextureFormats(int format, unsigned int *glInternalFormat, unsigned int *glFormat, unsigned int *glType)
{
    ...
    *glType = 0;
    ...
    switch (format)
    {
        ...
        #if !defined(GRAPHICS_API_OPENGL_11)
        #if defined(GRAPHICS_API_OPENGL_ES3)
        case RL_PIXELFORMAT_UNCOMPRESSED_R32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_R32F_EXT; *glFormat = GL_RED_EXT; *glType = GL_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_RGB32F_EXT; *glFormat = GL_RGB; *glType = GL_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_RGBA32F_EXT; *glFormat = GL_RGBA; *glType = GL_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_R16F_EXT; *glFormat = GL_RED_EXT; *glType = GL_HALF_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_RGB16F_EXT; *glFormat = GL_RGB; *glType = GL_HALF_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_RGBA16F_EXT; *glFormat = GL_RGBA; *glType = GL_HALF_FLOAT; break;
        #else
        case RL_PIXELFORMAT_UNCOMPRESSED_R32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_LUMINANCE; *glFormat = GL_LUMINANCE; *glType = GL_FLOAT; break;            // NOTE: Requires extension OES_texture_float
        case RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_RGB; *glFormat = GL_RGB; *glType = GL_FLOAT; break;                  // NOTE: Requires extension OES_texture_float
        case RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_RGBA; *glFormat = GL_RGBA; *glType = GL_FLOAT; break;             // NOTE: Requires extension OES_texture_float
        #if defined(GRAPHICS_API_OPENGL_21)
        case RL_PIXELFORMAT_UNCOMPRESSED_R16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_LUMINANCE; *glFormat = GL_LUMINANCE; *glType = GL_HALF_FLOAT_ARB; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_RGB; *glFormat = GL_RGB; *glType = GL_HALF_FLOAT_ARB; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_RGBA; *glFormat = GL_RGBA; *glType = GL_HALF_FLOAT_ARB; break;
        #else // GRAPHICS_API_OPENGL_ES2
        case RL_PIXELFORMAT_UNCOMPRESSED_R16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_LUMINANCE; *glFormat = GL_LUMINANCE; *glType = GL_HALF_FLOAT_OES; break;   // NOTE: Requires extension OES_texture_half_float
        case RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_RGB; *glFormat = GL_RGB; *glType = GL_HALF_FLOAT_OES; break;         // NOTE: Requires extension OES_texture_half_float
        case RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_RGBA; *glFormat = GL_RGBA; *glType = GL_HALF_FLOAT_OES; break;    // NOTE: Requires extension OES_texture_half_float
        #endif
        #endif
        #endif
    #elif defined(GRAPHICS_API_OPENGL_33)
        ...
        case RL_PIXELFORMAT_UNCOMPRESSED_R32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_R32F; *glFormat = GL_RED; *glType = GL_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_RGB32F; *glFormat = GL_RGB; *glType = GL_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R32G32B32A32: if (RLVK.ExtSupported.texFloat32) *glInternalFormat = GL_RGBA32F; *glFormat = GL_RGBA; *glType = GL_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_R16F; *glFormat = GL_RED; *glType = GL_HALF_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_RGB16F; *glFormat = GL_RGB; *glType = GL_HALF_FLOAT; break;
        case RL_PIXELFORMAT_UNCOMPRESSED_R16G16B16A16: if (RLVK.ExtSupported.texFloat16) *glInternalFormat = GL_RGBA16F; *glFormat = GL_RGBA; *glType = GL_HALF_FLOAT; break;
    #endif
    #if !defined(GRAPHICS_API_OPENGL_11)
        case RL_PIXELFORMAT_COMPRESSED_DXT1_RGB: if (RLVK.ExtSupported.texCompDXT) *glInternalFormat = GL_COMPRESSED_RGB_S3TC_DXT1_EXT; break;
        case RL_PIXELFORMAT_COMPRESSED_DXT1_RGBA: if (RLVK.ExtSupported.texCompDXT) *glInternalFormat = GL_COMPRESSED_RGBA_S3TC_DXT1_EXT; break;
        case RL_PIXELFORMAT_COMPRESSED_DXT3_RGBA: if (RLVK.ExtSupported.texCompDXT) *glInternalFormat = GL_COMPRESSED_RGBA_S3TC_DXT3_EXT; break;
        case RL_PIXELFORMAT_COMPRESSED_DXT5_RGBA: if (RLVK.ExtSupported.texCompDXT) *glInternalFormat = GL_COMPRESSED_RGBA_S3TC_DXT5_EXT; break;
        case RL_PIXELFORMAT_COMPRESSED_ETC1_RGB: if (RLVK.ExtSupported.texCompETC1) *glInternalFormat = GL_ETC1_RGB8_OES; break;                      // NOTE: Requires OpenGL ES 2.0 or OpenGL 4.3
        case RL_PIXELFORMAT_COMPRESSED_ETC2_RGB: if (RLVK.ExtSupported.texCompETC2) *glInternalFormat = GL_COMPRESSED_RGB8_ETC2; break;               // NOTE: Requires OpenGL ES 3.0 or OpenGL 4.3
        case RL_PIXELFORMAT_COMPRESSED_ETC2_EAC_RGBA: if (RLVK.ExtSupported.texCompETC2) *glInternalFormat = GL_COMPRESSED_RGBA8_ETC2_EAC; break;     // NOTE: Requires OpenGL ES 3.0 or OpenGL 4.3
        case RL_PIXELFORMAT_COMPRESSED_PVRT_RGB: if (RLVK.ExtSupported.texCompPVRT) *glInternalFormat = GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG; break;    // NOTE: Requires PowerVR GPU
        case RL_PIXELFORMAT_COMPRESSED_PVRT_RGBA: if (RLVK.ExtSupported.texCompPVRT) *glInternalFormat = GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG; break;  // NOTE: Requires PowerVR GPU
        case RL_PIXELFORMAT_COMPRESSED_ASTC_4x4_RGBA: if (RLVK.ExtSupported.texCompASTC) *glInternalFormat = GL_COMPRESSED_RGBA_ASTC_4x4_KHR; break;  // NOTE: Requires OpenGL ES 3.1 or OpenGL 4.3
        case RL_PIXELFORMAT_COMPRESSED_ASTC_8x8_RGBA: if (RLVK.ExtSupported.texCompASTC) *glInternalFormat = GL_COMPRESSED_RGBA_ASTC_8x8_KHR; break;  // NOTE: Requires OpenGL ES 3.1 or OpenGL 4.3
    #endif
...
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if (RLVK.ExtSupported.vao) glGenVertexArrays(1, &vaoId);
#endif
...
unsigned int rlLoadShaderProgram(const char *vsCode, const char *fsCode)
{
    ...
    else vertexShaderId = RLVK.State.defaultVShaderId;

    // Compile fragment shader (if provided)
    // NOTE: If not vertex shader is provided, use default one
    if (fsCode != NULL) fragmentShaderId = rlLoadShader(fsCode, GL_FRAGMENT_SHADER);
    else fragmentShaderId = RLVK.State.defaultFShaderId;

    // In case vertex and fragment shader are the default ones, no need to recompile, assign the default shader program id
    if ((vertexShaderId == RLVK.State.defaultVShaderId) && (fragmentShaderId == RLVK.State.defaultFShaderId)) id = RLVK.State.defaultShaderId;
    else if ((vertexShaderId > 0) && (fragmentShaderId > 0))
    {
        ...
        if (vertexShaderId != RLVK.State.defaultVShaderId)
        {
            // WARNING: Shader program linkage could fail and returned id is 0
            if (id > 0) glDetachShader(id, vertexShaderId);
            glDeleteShader(vertexShaderId);
        }
        if (fragmentShaderId != RLVK.State.defaultFShaderId)
        {
            // WARNING: Shader program linkage could fail and returned id is 0
...
void rlSetUniformSampler(int locIndex, unsigned int textureId)
{
    ...
    for (int i = 0; i < RL_DEFAULT_BATCH_MAX_TEXTURE_UNITS; i++)
    {
        if (RLVK.State.activeTextureId[i] == textureId)
        {
    ...
    for (int i = 0; i < RL_DEFAULT_BATCH_MAX_TEXTURE_UNITS; i++)
    {
        if (RLVK.State.activeTextureId[i] == 0)
        {
            ...
            RLVK.State.activeTextureId[i] = textureId; // Save texture id for binding on drawing
            break;
        }
    }
#endif
...
// NOTE: Loaded: RLVK.State.defaultShaderId, RLVK.State.defaultShaderLocs
static void rlLoadShaderDefault(void)
{
    RLVK.State.defaultShaderLocs = (int *)RL_CALLOC(RL_MAX_SHADER_LOCATIONS, sizeof(int));

    // NOTE: All locations must be reseted to -1 (no location)
    for (int i = 0; i < RL_MAX_SHADER_LOCATIONS; i++) RLVK.State.defaultShaderLocs[i] = -1;
    ...
    RLVK.State.defaultVShaderId = rlLoadShader(defaultVShaderCode, GL_VERTEX_SHADER);     // Compile default vertex shader
    RLVK.State.defaultFShaderId = rlLoadShader(defaultFShaderCode, GL_FRAGMENT_SHADER);   // Compile default fragment shader

    RLVK.State.defaultShaderId = rlLoadShaderProgramEx(RLVK.State.defaultVShaderId, RLVK.State.defaultFShaderId);

    if (RLVK.State.defaultShaderId > 0)
    {
        TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] Default shader loaded successfully", RLVK.State.defaultShaderId);

        // Set default shader locations: attributes locations
        RLVK.State.defaultShaderLocs[RL_SHADER_LOC_VERTEX_POSITION] = glGetAttribLocation(RLVK.State.defaultShaderId, RL_DEFAULT_SHADER_ATTRIB_NAME_POSITION);
        RLVK.State.defaultShaderLocs[RL_SHADER_LOC_VERTEX_TEXCOORD01] = glGetAttribLocation(RLVK.State.defaultShaderId, RL_DEFAULT_SHADER_ATTRIB_NAME_TEXCOORD);
        RLVK.State.defaultShaderLocs[RL_SHADER_LOC_VERTEX_COLOR] = glGetAttribLocation(RLVK.State.defaultShaderId, RL_DEFAULT_SHADER_ATTRIB_NAME_COLOR);

        // Set default shader locations: uniform locations
        RLVK.State.defaultShaderLocs[RL_SHADER_LOC_MATRIX_MVP] = glGetUniformLocation(RLVK.State.defaultShaderId, RL_DEFAULT_SHADER_UNIFORM_NAME_MVP);
        RLVK.State.defaultShaderLocs[RL_SHADER_LOC_COLOR_DIFFUSE] = glGetUniformLocation(RLVK.State.defaultShaderId, RL_DEFAULT_SHADER_UNIFORM_NAME_COLOR);
        RLVK.State.defaultShaderLocs[RL_SHADER_LOC_MAP_DIFFUSE] = glGetUniformLocation(RLVK.State.defaultShaderId, RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE0);
    }
    else TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Failed to load default shader", RLVK.State.defaultShaderId);
}

// Unload default shader
// NOTE: Unloads: RLVK.State.defaultShaderId, RLVK.State.defaultShaderLocs
static void rlUnloadShaderDefault(void)
{
    glUseProgram(0);

    glDetachShader(RLVK.State.defaultShaderId, RLVK.State.defaultVShaderId);
    glDetachShader(RLVK.State.defaultShaderId, RLVK.State.defaultFShaderId);
    glDeleteShader(RLVK.State.defaultVShaderId);
    glDeleteShader(RLVK.State.defaultFShaderId);

    glDeleteProgram(RLVK.State.defaultShaderId);

    RL_FREE(RLVK.State.defaultShaderLocs);

    TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] Default shader unloaded successfully", RLVK.State.defaultShaderId);
}
